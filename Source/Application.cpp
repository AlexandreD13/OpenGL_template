...
void Application::displayOverlay(bool display, float milli, float seconds)
{
	...
	ImGui::Text("Number of Effects: %lu", m_scene->m_Effects.size());
	ImGui::Text("Number of Models: %lu", m_scene->m_Models.size());
	ImGui::Text("Number of Nodes: %lu", m_scene->m_Nodes.size());
	ImGui::Separator();
	...
	if (ImGui::BeginPopup("Background Color"))
	{
		...
		m_engine->setClearColor(glm::vec4(backgroundColor.x, backgroundColor.y, backgroundColor.z, backgroundColor.w));
		ImGui::EndPopup();
	}
