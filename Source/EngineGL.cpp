...
void message_callback(GLenum source, GLenum type, GLuint id, GLenum severity, __attribute__((unused)) GLsizei length,
                      GLchar const* message, __attribute__((unused)) void const* user_param) {
...
bool EngineGL::init() {
	...
	auto* sphereMaterial = new BaseMaterial("Sphere");
	auto* rotationMaterial = new RotationMaterial("Rotation");
	auto* phongMaterial = new PhongMaterial("Phong");
	auto* phongMaterialWall = new PhongMaterial("PhongWall");
	...
	if (myFBO) {
		if (ImGui::BeginMainMenuBar()) {
			if (ImGui::BeginMenu("FBOs")) {
				ImGui::MenuItem(myFBO->getName().c_str(), nullptr, &(myFBO->show_interface));
				ImGui::EndMenu();
			}
